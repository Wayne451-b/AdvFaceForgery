...
def save_grid_img(imgs, save_root_path, epoch):
    ...
    out_file = os.path.join(save_root_path, f'{epoch}_result.png')
    x_concat = torch.cat(imgs, dim=-1)
    vutils.save_image(x_concat.data, out_file, nrow=1, normalize=True)
...
def full_noise_clamp(adv_noise, img_size, sa_mask):
...
def noise_clamp(adv_noise, img_size, sa_mask):
    half_size = img_size
    if sa_mask.size() != adv_noise.size():
        sa_mask = F.interpolate(sa_mask, size=adv_noise.shape[2:], mode='bilinear', align_corners=False)
    adv_noise = adv_noise[:, :, :, :].clone() * sa_mask
    return adv_noise


def tensor2numpy(x):
    img = (x[0] / 2 + 0.5).mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
    return img
