...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02,
             gpu_ids=[], transf_enc_pos=[4], transf_dec_pos=[4], use_mask = True):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)

    if netG == 'unet_64':
        net = UnetGenerator(input_nc, output_nc, 4, ngf,
...
class UnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
                 ...
                 transf_enc_pos=[], transf_dec_pos=[], use_mask=True):
        super().__init__()
    ...
    def forward(self, original_img, reference_img, mask):
        ...
        phase_swapped_features = phase_swapped_feature * mask
        result = self.model(phase_swapped_features)
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        super().__init__()
        assert not (outermost and innermost), "cannot be both outermost and innermost"
        ...
        if outermost:
            ...
            up_high_freq_aware = Adp_spa_freq_attention(feat_dim=outer_nc, patch_size=16)
            down = [downconv]
            up = [uprelu, upconv, up_high_freq_aware, nn.Tanh()]
